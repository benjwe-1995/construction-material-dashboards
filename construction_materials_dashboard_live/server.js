...
function parseFredCsv(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (Number.isFinite(value)) data.push({ date, value });
  }
  return data;
}

async function handleFred(req, res, series) {
  ...
  try {
    const response = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 construction-dashboard-local' } });
    if (!response.ok) throw new Error(`FRED HTTP ${response.status}`);
    const csv = await response.text();
    const data = parseFredCsv(csv);
    send(res, 200, JSON.stringify({ series, source_url: url, data }));
  } catch (err) {
    send(res, 502, JSON.stringify({ error: err.message, series, source_url: url }));
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (parsed.pathname.startsWith('/api/fred/')) {
    ...
    return handleFred(req, res, series);
  }
...
server.listen(PORT, () => {
  console.log(`Construction materials dashboard running at http://localhost:${PORT}`);
});
